...
function search(username) {
	for (var score of scores) {
		if (score.username.toLowerCase() == username.toLowerCase()) {
			...
			document.getElementById("search-name").innerHTML = score.username;
			document.getElementById("search-name").style.color = `rgb(${Math.round(155 * score.guesses.b / maxGuess) + 100}, ${Math.round(155 * score.guesses.a / maxGuess) + 100}, ${Math.round(155 * score.guesses.x / maxGuess) + 100})`;
			document.getElementById("search-wins").innerHTML = totalWins;
			document.getElementById("search-guesses").innerHTML = totalGuesses;
			let a = document.getElementById("search-accuracy");
			...
			document.getElementById("search-guesses-x").innerHTML = score.guesses.x;
			document.getElementById("search-guesses-a").innerHTML = score.guesses.a;
			document.getElementById("search-guesses-b").innerHTML = score.guesses.b;
